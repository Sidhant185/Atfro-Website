...
- **503 / ChunkLoadError / 404 + MIME type for JS:** If the browser reports "Failed to load chunk", 404 for `/_next/static/chunks/*.js`, or "Refused to execute script... MIME type ('text/plain')", the HTML was generated by a different build than the one currently on the server. **After every deploy (e.g. after `git pull`), you must run a full `npm run build` and then restart the app.** Do not skip the build or reuse an old `.next` folder. The full `.next` folder (including `static/chunks`) must match the running app. Use Node 20.x.

---

## Production Checklist

- [ ] Node.js app created on Hostinger (20.x or 22.x; do not use 18.x)
